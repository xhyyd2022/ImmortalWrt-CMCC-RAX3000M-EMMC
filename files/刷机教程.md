...
5. 保险起见可以自己先在电脑上计算一下原始文件的哈希值，上传后的文件和分区的哈希值，避免传输或刷入异常造成文件损坏。
...
7. 将`immortalwrt-mediatek-filogic-cmcc_rax3000m-initramfs-recovery.itb`放到到TFTP服务器根目录，或手动选择文件路径，运行tftpd64.exe，指定服务接口为设置的电脑网卡。
8. (可选，查看过程）连接ttl网口朝外分别为GND,TX,VCC,RX，连接USB转TTL线，GND连GND，TX连RX，VCC留空，RX连TX。
9. 按住reset键重启进入U-Boot，会触发自动启动TFTP固件传输。
...
7. 现在系统被初始化，空间变大，一切从头开始配置就可以了

   注：之所以开始配置好后不直接备份配置，是考虑到是把p6作为Overlay，p5给闲置了，如果备份又恢复配置，那么有可能又变回p5作为Overlay了。