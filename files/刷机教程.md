...
2. 流程
   ...
   - 刷入BL2：清空并写入引导加载程序
   - 刷入FIP：清空并写入第二阶段引导程序
   - 在同文件目录下直接执行或创建一键脚本进行刷入操作：`flash.sh`
   ```bash
   #!/bin/sh
   ...
   # 备份 GPT 分区表（前 34 个扇区）
   dd if=/dev/mmcblk0 bs=512 count=34 of=backup/mmcblk0-gpt-backup.bin conv=fsync

   # 备份 BL2（boot0 区域前 8192 个扇区）
   dd if=/dev/mmcblk0boot0 bs=512 count=8192 of=backup/mmcblk0boot0-bl2-backup.bin conv=fsync

   # 备份 FIP（从 13312 开始，长度 8192 个扇区）
   dd if=/dev/mmcblk0 bs=512 skip=13312 count=8192 of=backup/mmcblk0-fip-backup.bin conv=fsync

   echo "✅ 所有关键分区备份完成，保存在 ./backup 目录"
...
3. 执行命令刷入
   ...
   ```
4. 保险起见可以计算上传后的文件和分区的哈希值，比较哈希值与`sha256sums`文件里对应的哈希值是否一样，避免传输或刷入异常造成文件损坏。
   ```bash
   #!/bin/sh
   cd /tmp
   ...
   echo "验证 BL2..."
   BL2=immortalwrt-mediatek-filogic-cmcc_rax3000m-emmc-preloader.bin
   size=$(wc -c < "$BL2")
   dd if=/dev/mmcblk0boot0 bs=1 count=$size | sha256sum
   sha256sum "$BL2"

   echo "验证 FIP..."
   FIP=immortalwrt-mediatek-filogic-cmcc_rax3000m-emmc-bl31-uboot.fip
   size=$(wc -c < "$FIP")
   dd if=/dev/mmcblk0 bs=1 skip=$((13312*512)) count=$size | sha256sum
   sha256sum "$FIP"
   ```
5. 手动配置电脑网络：IP：`192.168.1.254`，掩码：`255.255.255.0`，网关：`192.168.1.1`
6. 将`immortalwrt-mediatek-filogic-cmcc_rax3000m-initramfs-recovery.itb`放到到TFTP服务器根目录，或手动选择文件路径，运行`tftpd64.exe`，指定服务接口为设置的电脑网卡。
7. 连接ttl网口朝外分别为GND,TX,VCC,RX，连接USB转TTL线，GND连GND，TX连RX，VCC留空，RX连TX。（可选，查看过程）
...
8. 修改电脑网卡为自动获取IP，访问`192.168.2.1`进入系统。
9. 在`系统→备份与更新→更新固件`中，选择`immortalwrt-mediatek-filogic-cmcc_rax3000m-squashfs-sysupgrade.itb`刷入正式系统。
10. 后续升级，直接参考步骤9刷入新系统。
...
2. 修改lan口
   ...
   - 高级设置
     - 强制连接：关闭
     - 使用该接口的网关作为默认网关：关闭（纯AP模式，关闭后自己本身无法联网）
...
4. 直接扩容分区5（方式一）
   ...
   ```
   - 来到`系统→挂载点→生成配置`，重启系统。
   - 不行的话进入`parted`重新操作，输入`p`输出当前分区查看是否变更，点击`生成配置`并重启查看是否扩容。

5. 创建新分区（方式二，从剩余空间开始）
   - 查看之前输出的p5是`/dev/mmcblk0p5   131072 458751  327680  160M unknown`
   - 剩余空间起点是：
      ```bash
      Start = 458752 (p5的终止458751，p6的起始+1)
      ```
